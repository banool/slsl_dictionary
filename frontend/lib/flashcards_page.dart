...
class FlashcardsPageState extends State<FlashcardsPage> {
  ...
  Widget buildFlashcardWidget(
    ...
    // SLSL's regional information (region names pretty-printed in the current
    // language), flanked by plain (no-fill) back/forward arrows absolutely
    // positioned on either side. Back goes to the previous card; forward only
    // lights up once you've stepped back and there's a later card to return to.
    // Shown in both states so navigation is always available; the region text
    // itself is hidden until the card is revealed.
          ...
          Positioned.fill(
            child: Center(
              child: Padding(
                ...
                child: getRegionalInformationWidget(
