...
// End-to-end coverage for the word page's "saved to N lists" count (ported from
// auslan_dictionary, adapted for SLSL). It is computed from the same
// writable-list set the save sheet shows (ListsService.writableLists), so the
// label and the sheet's ticked rows can't disagree: saving a video into two
// lists reads back as "Saved to 2 lists", and clearing both drops the button
// back to "Save".
...
void main() {
  ...
  testWidgets(
    ...
    // A deterministic entry: one sub-entry (single bookmark button, no PageView
    // siblings) with at least one media item. The button saves the first shown.
