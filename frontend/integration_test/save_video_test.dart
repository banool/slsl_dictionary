...
// entries" feature (ported from auslan_dictionary, adapted for SLSL). It drives
// the real per-video bookmark button, the real save-to-list sheet, and asserts
// the toggle both persists to disk (a fresh read sees it) and surfaces in the
// list view.
//
...
// sheet) rather than by typing in search, so the test targets a known entry and
// isn't at the mercy of search-result ordering / lazy list building.
...
void main() {
  ...
  testWidgets(
    ...
    // Don't let the once-per-session advisories interstitial push the News page
    // over the search screen and derail navigation.
    ...
    // The button saves the currently-shown media, which is the first
    ...
    // screen is reachable. Both routes sit on the root navigator, so pop twice.
