...
// Guards the alignment of the Hearth single-choice picker (used by the App
// theme / Colour mode settings rows), ported from auslan_dictionary. The bug
// this protects against: the option rows sat at a 15dp inset while the dialog
// title used the Material 24dp inset, so the options looked outdented to the
// left of the title. The fix lines the option labels up under the title; this
// test asserts their left edges match — and, for SLSL, that wiring
// themeVariantNotifier surfaces the Hearth/Classic picker at all.
...
void main() {
  ...
  testWidgets('app theme picker options line up under the dialog title',
    ...
    // Scope every lookup to the dialog: the settings row behind the scrim also
    // shows "App theme" (its title) and "Hearth" (its current-value trailing),
    // so an unscoped find.text would match two widgets.
